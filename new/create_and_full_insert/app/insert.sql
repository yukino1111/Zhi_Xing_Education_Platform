...
insert overwrite table app_didi.t_order_subscribe_percent partition(month='2020-04')
select
    '2020-04-12' as dt_col,
    '预约' as type_col,
    concat(round(sum(case when subscribe = 1 then 1 else 0 end) / count(orderid) * 100, 2), '%') as percent_val
from
    dw_didi.t_user_order_wide
where
    dt = '2020-04-12'
...
select
    '2020-04-12' as dt_col,
    '非预约' as type_col,
    concat(round(sum(case when subscribe = 0 then 1 else 0 end) / count(orderid) * 100, 2), '%') as percent_val
from
    dw_didi.t_user_order_wide
where
    dt = '2020-04-12';
